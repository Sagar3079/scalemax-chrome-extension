var webEditorV2 = (function() {
	//#region ../../node_modules/.pnpm/wxt@0.20.27_@types+node@22._c13784526c76306bd4ecdeabada10fa4/node_modules/wxt/dist/utils/define-unlisted-script.mjs
...
`;
	...
	*/
	function installFloatingDrag(options) {
		...
		function applyNextPosition(next) {
			...
			const clamped = clampPosition(next, s ? {
				width: s.targetWidth,
	...
	*/
	function createToolbar(options) {
		...
		const DISALLOWED_TARGET_TAGS = /* @__PURE__ */ new Set([
			"HTML",
			"BODY",
			"HEAD"
		]);
	...
	*/
	...
	var LENGTH_KEYWORDS = /* @__PURE__ */ new Set([
		"auto",
		"inherit",
		"initial",
		"unset",
	...
	function parseTransform(transform) {
		...
		for (let i = 0; i < functions.length; i++) switch (functions[i].name) {
			...
			case "scalex":
				...
				break;
			case "scaley": if (scaleYIndex === -1) scaleYIndex = i;
		}
		return {
			functions,
			rotateIndex,
	...
	*/
	function setRotateValue(state, value) {
		...
		if (state.rotateIndex >= 0) {
			if (rotateStr) state.functions[state.rotateIndex] = {
...
`;
	...
	*/
	function normalizeStopPositions(stops) {
		...
		for (let i = 1; i < positions.length; i++) {
			...
			if (pos !== null) {
				if (pos < maxSoFar) positions[i] = maxSoFar;
				else maxSoFar = pos;
			}
		}
		let runStart = null;
	...
	var UNSUPPORTED_RADIAL_SIZE_KEYWORDS = /* @__PURE__ */ new Set([
		"closest-side",
		"farthest-side",
		"closest-corner",
		"farthest-corner"
	...
	function createBorderEdgeIcon(edge) {
		...
		switch (edge) {
			...
			case "bottom":
				...
				break;
			case "left": highlight.setAttribute("d", "M3.5 3.5v8");
		}
	...
	function createCornerIcon(corner) {
		...
		switch (corner) {
			...
			case "bottom-right":
				...
				break;
			case "bottom-left": path.setAttribute("d", "M4 4V9Q4 11 6 11H11");
		}
		svg.appendChild(path);
		return svg;
	}
	...
	*/
	function formatElementLabel$1(element) {
		...
		if (!element.children.length) {
			...
			else if (text.length > MAX_TEXT_LENGTH) label += ` "${text.slice(0, 17)}..."`;
		}
		return label;
	}
	/**
	...
	*/
	...
	var _globalThis$CSSRule;
	var _globalThis$CSSRule2;
	var ZERO_SPEC = [
		0,
		0,
		0,
	...
	*/
	var GLOBAL_SELECTORS = /* @__PURE__ */ new Set([
		"*",
		"html",
		"body",
		":root",
	...
	*/
	function readComponentName(value) {
		...
		if (typeof value === "function") {
			var _readString;
			const fn = value;
			return (_readString = readString(fn.displayName)) !== null && _readString !== void 0 ? _readString : readString(fn.name);
		}
		const rec = asRecord(value);
		if (rec) {
			var _readString2;
			return (_readString2 = readString(rec.displayName)) !== null && _readString2 !== void 0 ? _readString2 : readString(rec.name);
		}
	}
	/**
	...
	*/
	function extractReactDebugSource(fiber) {
		...
		for (let i = 0; i < MAX_FIBER_DEPTH && current; i++) {
			...
			const file = readString(src === null || src === void 0 ? void 0 : src.fileName);
			if (file) {
			...
			const ownerFile = readString(ownerSrc === null || ownerSrc === void 0 ? void 0 : ownerSrc.fileName);
			if (ownerFile) {
		...
		try {
			...
			if (inspector === null || inspector === void 0 ? void 0 : inspector.file) {
				...
				for (let depth = 0; depth < MAX_DOM_DEPTH && node; depth++) {
					...
					componentName = readString(typeRec === null || typeRec === void 0 ? void 0 : typeRec.name);
			...
			for (let depth = 0; depth < MAX_DOM_DEPTH && node; depth++) {
				const inst = asRecord(node.__vueParentComponent);
				const typeRec = asRecord(inst === null || inst === void 0 ? void 0 : inst.type);
				const file = readString(typeRec === null || typeRec === void 0 ? void 0 : typeRec.__file);
				if (file) return {
					file,
					componentName: readString(typeRec === null || typeRec === void 0 ? void 0 : typeRec.name)
				};
	...
	*/
	function computeFingerprint(element) {
		...
		const text = normalizeText$2((_element$textContent = element.textContent) !== null && _element$textContent !== void 0 ? _element$textContent : "", FINGERPRINT_TEXT_MAX_LENGTH);
	...
	*/
	...
	var DANGEROUS_PROP_KEYS = /* @__PURE__ */ new Set([
		"__proto__",
		"constructor",
		"prototype",
		"__defineGetter__",
    ...
    <path d="M3.5 2.5H9.5V8.5M9 3L3 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
		...
		*/
		...
		function renderMeta() {
			...
			const componentName = lastData === null || lastData === void 0 ? void 0 : lastData.componentName;
	...
	*/
	function computeResizeSnap(params) {
		...
		if (fixedEdgeX) {
			if (lockX) {
				if (!allowedXTypes.includes(lockX.type)) lockX = null;
				else {
		...
		if (fixedEdgeY) {
			if (lockY) {
				if (!allowedYTypes.includes(lockY.type)) lockY = null;
				else {
	...
	*/
	function generateElementLabel(element) {
		...
		const text = normalizeText$1((_element$textContent = element.textContent) !== null && _element$textContent !== void 0 ? _element$textContent : "");
	...
	*/
	...
	var APPLICABLE_TX_TYPES = /* @__PURE__ */ new Set([
		"style",
		"text",
		"class"
	]);
	...
	*/
	...
	var TERMINAL_EXEC_STATUSES = /* @__PURE__ */ new Set([
		"completed",
		"failed",
		"error",
		"timeout",
			...
			*/
			...
			for (let sheetIndex = 0; sheetIndex < styleSheets.length; sheetIndex++) {
				...
				walkRules(cssRules, {
					sheetIndex,
					source: sheetSource,
					visited: /* @__PURE__ */ new Set([sheet])
				});
			}
			const stats = {
				styleSheets: styleSheets.length,
	...
	*/
	function createDesignTokensService(options = {}) {
		...
		function getOrCollectIndex(root) {
			...
			if (cached) {
				if (cacheMaxAgeMs > 0) {
					if (getNow() - cached.collectedAt >= cacheMaxAgeMs) invalidateRoot(root, "ttl");
			...
			_revertElement = _asyncToGenerator(function* (elementKey) {
				...
				try {
					...
					if (styleChanges) {
						...
						const properties = Array.from(/* @__PURE__ */ new Set([...Object.keys(before), ...Object.keys(after)])).map((p) => String(p !== null && p !== void 0 ? p : "").trim()).filter(Boolean);
