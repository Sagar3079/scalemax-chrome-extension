...
if (window.__KEYBOARD_HELPER_INITIALIZED__) {
  ...
  function parseSingleKeyCombination(keyString) {
    ...
    if (mainKeyPart.length === 1) {
      ...
      let _code=`Key${mainKeyPart.toUpperCase()}`;
      if(mainKeyPart.match(/^[0-9]$/)) _code=`Digit${mainKeyPart}`;
      else if(mainKeyPart===' ') return {key:' ',code:'Space',keyCode:32, charCode:32, modifiers};
      return {
        key: keyChar,
        code: _code,
        keyCode: charCode,
  ...
  function dispatchKeyEvents(parsedKeyInfo, element) {
    ...
    try {
      if(key.length===1 && !modifiers.ctrlKey && !modifiers.altKey && !modifiers.metaKey){
        try{
          const isEd = element.isContentEditable || (element.getAttribute&&element.getAttribute('contenteditable')!==null) || element.tagName==='INPUT'||element.tagName==='TEXTAREA';
          if(isEd){
            try{ element.dispatchEvent(new InputEvent('beforeinput',{bubbles:true,cancelable:true,inputType:'insertText',data:key})); }catch(e){}
            let ins=false; try{ ins=document.execCommand('insertText',false,key); }catch(e){}
            if(!ins && element.tagName==='INPUT'){ try{ const s=element.selectionStart; const e=element.selectionEnd; const v=element.value; element.value=v.slice(0,s)+key+v.slice(e); element.setSelectionRange(s+1,s+1); ins=true; }catch(e){} }
            try{ element.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:key})); }catch(e){}
          }
        }catch(e){}
      }
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    if (request.action === 'simulateKeyboard') {
      ...
      // Respond that it's initialized
      sendResponse({
        status: 'pong',
        initialized: true,
        v: PROTOCOL_VERSION,
        script: 'keyboard-helper',
      });
