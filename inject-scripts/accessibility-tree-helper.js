...
(function () {
  ...
  // restarts the counter at 0. Without an epoch, a `ref_5` minted before the
  // navigation silently resolves against whatever element now occupies slot 5 --
  // the agent clicks the wrong thing and reports success. Binding every minted
  ...
  /**
   * Mint a fresh epoch-qualified ref id: `ref_<epoch>_<n>`.
   * @returns {string}
   */
  function mintRefId() {
  ...
  /**
   * Extract the epoch embedded in a ref id.
   * Returns null for the legacy `ref_<n>` form (kept for backward compatibility),
   * which carries no epoch and therefore cannot be classified as stale.
   * @param {string} ref
   * @returns {string|null}
   */
  function refEpochOf(ref) {
    const m = /^ref_([^_]+)_(\d+)$/.exec(String(ref || ''));
    return m ? m[1] : null;
  }

  /**
   * Build a distinct, actionable error message for a ref that failed to resolve.
   * A cross-epoch miss means the page navigated (re-read the page); a same-epoch
   * miss means the element itself is gone. The caller must be able to tell those
   * two cases apart.
   * @param {string} ref
   * @returns {string}
   */
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    try {
      if (request && request.action === 'chrome_read_page_ping') {
        sendResponse({ status: 'pong', v: PROTOCOL_VERSION, script: 'accessibility-tree-helper' });
        return false;
      }
