...
if (window.__NETWORK_CAPTURE_HELPER_INITIALIZED__) {
  ...
  // copy of this script after an extension update and force a re-injection /
  // tab reload instead of silently talking to old handlers.
    ...
    try {
      ...
      if (method !== 'GET' && method !== 'HEAD' && formDataDescriptor) {
        ...
        try {
          ...
          // Abort instead of sending a partially-built / empty multipart body,
          // which the server would happily accept as a well-formed request.
          const message = e && e.message ? e.message : String(e);
          console.warn('Failed to construct FormData:', e);
          return {
            success: false,
            error: 'Failed to construct multipart body: ' + message,
          };
        }
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    ...
    if (request.action === 'chrome_network_request_ping') {
      sendResponse({ status: 'pong', v: PROTOCOL_VERSION, script: 'network-helper' });
